...
| **L1** 组件版本 | webmssdk `1.0.0.20`、sdk-glue `1.0.0.64-fix.01`、bdms `1.0.1.19-fix.01`、verifycenter `1.0.0.413/417`、captcha 构建 `4.0.28`、rmc-nocaptcha `1.0.0.51`、secsdk `25.4.3.1`（URL 内嵌版本，**404 = 已换代**；byteimg 桶名 `tos-cn-i-9r5gewecjs` 也可能换） | 风控 SDK 换代（必修）；验证码 SDK 换代（策略信号） | README 版本对照、`capture/`、URL 清单 |
| **L1b** 加载路径与内嵌版本常量 | sdk-glue 内嵌 `bdmsVersion="1.0.1.19-fix.01"` / `captchaVersion="4.0.10"` / `sdkGlueVersion="1.0.0.64-fix.01"`，并按 `…/rc-client-security/web/stable/<bdmsVersion>/bdms.js` 在 **两个 CDN**（`lf-c-flwb.bytetos.com`、`lf-headquarters-speed.yhgfb-cn-static.com`）加载 bdms | 常量先于 404 暴露换代意图；路径模板变 = 加载链改版 | 同 L1；两处 stable 地址已纳入 `track_douyin.py` |
