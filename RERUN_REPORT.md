...
| 11 | Python 移植 | `python3 abogus_py.py`（忠实路线）/ `abogus_full.py`（旧路线） | ⏳ 长度 168 两侧一致；5 次 SM3 输入、37 次熵抽取、程序 280 入参、环境校验和/属性特性均已对齐，**仅剩 4 个字符差异**（`s[12]` 的可写性探针已随属性特性位建模而一致；剩余集中在辅助程序 697 的输出，见 §五 与 [PY_PORT_REPORT.md](PY_PORT_REPORT.md) §九之二~九之八） |
