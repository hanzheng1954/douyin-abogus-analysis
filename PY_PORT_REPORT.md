...
### 九之九、能力布尔已排除：翻转实验（每槽独立进程）证明 Python 侧是对的

一开始用「同进程内换模块」做翻转实验，7 次结果完全相同 —— 那是**实验无效**（`abogus_py` 已被缓存，
后续 import 复用了第一次的 VM 模块）。改成**每个槽位一个独立进程**后结果可靠：

| 实验 | 差异位数 | 差异位置 |
|---|---|---|
| 不翻转（当前状态） | **4** | 34, 53, 54, 165 |
| 翻转 s[8] / s[9] | 9 | +37, 38, 41, 42, 166 |
| 翻转 s[10] / s[11] / s[12] | 9 | +58, 59, 62, 63, 166 |
| 翻转 s[13] / s[14] | 9 | +11, 12, 13, 36, 166 |

即**任何一个能力布尔翻转都会让结果更差** → 697 的 7 个环境能力布尔两侧一致，剩余 4 位不是它们造成的。

### 九之十、剩余 4 位的落点与下一步

`W697` 打点（仅覆盖 op 54 S_WRITE）只看到 pc≤121 的写入（s[2..15]），而 697 的字节码有 253 字节，
返回数组（150 里落到 slot 37 的 `[0,0,0,0,rand]`）是在 **pc≥122 的区间**装配的，那里没有 op 54 写入
→ 需要用 op 16（SET_TOP）/ op 20（SET_GK）以及 `concat`/数组字面量路径打点。
