...
试过「逐个翻转 697 的 7 个能力布尔，看哪个翻转后签名收敛」的快速实验，但 7 次结果**完全相同**（都是 4 位、
位置也一样）→ 说明该实验里的覆写注入根本没生效（不是结论），下次要做需先在 `S_WRITE` 分支确认打点命中
（先用 print 验证 pid/slot 命中再谈翻转），或改用「在 697 出口打印返回值 + 逐项与 Node 配对」的方式。

另外 Node 侧 697 的 7 个探测器返回值仍未取全：目前只拿到 691/714/720 的返回（走 f=2 帧返回路径），
716/704/724/722/727 既不走 f=2/f=3 帧返回、也不走 apply 结果路径，需要在 CALL 分支再加「帧切换后的返回值」
钩子（`g(y[0], …)` 之后的值由 `y()` 弹帧时压栈，钩点应放在弹帧分支并按 `h[h.length-1]` 判定调用者）。
