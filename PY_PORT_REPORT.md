...
**下一步最小动作（已缩小到一次插桩）**：在这两个数组的构建路径上打点 —— 它们不是由 150 的
`SET/SET_TOP/SET_GK` 直接按数字下标写入的（已试过，无命中），因此大概率由某个辅助程序
（`s[88]`/`s[90]` 的赋值点）或数组方法（push/concat/slice）产出。建议：在 150 内对 `S_WRITE chain=0 slot=88/90`
打点，记录赋值时源数组的内容与调用者 pid，再回到该程序里找写入下标 10/13 的那条指令，
对比两侧入栈值即可收口（预期 0 差异）。
