...
**下一步最小动作**：把 Node 侧同一处（150 pc=1684 的 concat 接收者 `d`，或 pc=1672 的 SLICE 结果）
dump 出来与上表逐项对齐，即可看到具体是哪两个栈值不同；再往回追这两项的产出指令
（都是 150 内部的 XOR/位混合结果，其输入端才是原始字段），即可收口到 0 差异。
（本轮的 Node 侧打点未命中：该处位于 `V.get(n)` 为假的 apply 分支，需要在 `d()` 里按
`__PIDMAP.get(o)===150 && a-2===1684` 打点，且要确认 `o` 仍是 `z[150][0]`。）
